...
public class WlanSeciritiesOld extends WlanSecurities {
	...
	public void setupSecurity(WifiConfiguration config, String security, final String password) {
		...
		MyHelper.d(">>>> setupSecurity old  security="+security);
		...
		if (security.equals(WEP)) {
			...
			if (!TextUtils.isEmpty(password)) {
				if (wepPasswordType == WEP_PASSWORD_AUTO) {
					if (isHexWepKey(password)) {
						...
						config.wepKeys[0] = WlanHelper.convertToQuotedString(password);
					}
				} else {
					config.wepKeys[0] = wepPasswordType == WEP_PASSWORD_ASCII
							? WlanHelper.convertToQuotedString(password) : password;
				}
			}
			...
			if (!TextUtils.isEmpty(password)) {
				if (password.length() == 64 && isHex(password)) {
					...
					config.preSharedKey = WlanHelper.convertToQuotedString(password);
				}
			}
