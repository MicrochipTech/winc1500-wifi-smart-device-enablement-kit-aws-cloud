...
import com.amazonaws.mchp.awsprovisionkit.activity.NetworkProvisionStageOneOneActivity;
...
public final class CaptureActivity extends Activity implements SurfaceHolder.Callback {
			...
			case START_BIND:
				...
				for (String string : strings) {
					if (caller.equals("networkProvisionActivity"))
						NetworkProvisionStageOneOneActivity.boundMessage.add(string);
	...
	protected  void onCreate(Bundle savedInstanceState) {
		...
		caller = intent.getStringExtra(ServiceConstant.CallerCmd);
		deviceType = intent.getStringExtra(ServiceConstant.DeviceType);
	...
	public void handleDecode(Result rawResult, Bundle bundle) {
		...
		if(text.length()== 12) {
			for(i=0; i<12; i++)
			{
				...
				if((data >= '0' && data <='9') || (data>='A' && data <='F'))
				{
						...
						Log.i("Apptest", "It is NOT MAC-address !");
						break;
				}
			}
		}
		...
		strings[0] = "QR Mac";
		strings[1] = (i == 12) ? text : "invalid";
		strings[2] = deviceType;
	...
	private void displayFrameworkBugMessageAndExit() {
		...
		String camera_error = "error";//getText("").toString();
		builder.setMessage(camera_error);
		String shure = "besure";//getText(R.string.besure).toString();
