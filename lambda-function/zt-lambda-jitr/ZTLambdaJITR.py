...
ZT_THING_TYPE_NAME = 'microchip-wifi-secure-board'
ZT_THING_GROUP_NAME = 'microchip-wifi-secure-board-group'
...
def lambda_handler(event, context):
    ...
    caCertificateId = event['caCertificateId']
    print('caCertificateId is {}'.format(caCertificateId))
    ...
    # Create thing group (no error if it already exists)
    response = iot.create_thing_group(thingGroupName=ZT_THING_GROUP_NAME)
    ...
    reponse = iot.add_thing_to_thing_group(
        thingGroupName=ZT_THING_GROUP_NAME,
        thingGroupArn='arn:aws:iot:{}:{}:thinggroup/{}'.format(region, account_id, ZT_THING_GROUP_NAME),
        thingName=thing_name,
        thingArn='arn:aws:iot:{}:{}:thing/{}'.format(region, account_id, thing_name))
